...
import { Calendar, Lock, AlertCircle, ArrowRight, Loader2, QrCode, Eye, EyeOff, CheckCircle2, Check, X, Users, TrendingUp, BookOpen, Clock, FileText, Award, ShieldAlert, Bell, UserCheck } from 'lucide-react';
...
export default function ResetPassword() {
  ...
  useEffect(() => {
    if (!token) {
      setError('Enlace de recuperación inválido: falta el token de seguridad.');
    }
  }, [token]);
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="text-center mb-6 flex flex-col items-center">
          ...
          <p className="text-txt-muted mt-1 text-xs lg:text-sm">
            Restablece el acceso a tu cuenta de RinoAsist.
          </p>
        </div>
        ...
        <div className="bg-bg-card/75 dark:bg-bg-card/65 backdrop-blur-xl border border-bdr-base/80 p-8 rounded-3xl shadow-2xl theme-transition relative">
          ...
          {success ? (
            ...
            <div className="text-center space-y-4 py-4 animate-scale-in">
              ...
              <h3 className="text-xl font-bold text-txt-base">Contraseña Guardada</h3>
              <p className="text-sm text-txt-muted">
                Tu contraseña se ha restablecido exitosamente. Redireccionando al inicio de sesión...
              </p>
              <button
            ...
            <form onSubmit={handleResetPassword} className="space-y-5">
              {/* Error Message */}
              {error && (
                <div className="bg-rose-500/10 border border-rose-500/30 text-rose-600 dark:text-rose-450 px-4 py-3 rounded-xl flex items-center gap-3 text-sm">
              ...
              <div className="space-y-2">
                ...
                {/* Password Requirements Checklist (Always visible for user guidance) */}
              ...
              <div className="space-y-2">
                ...
                {/* Password Match Status Alert (Enhanced Badge) */}
                {confirmPassword && (
                  <div className={`mt-2 p-2.5 rounded-xl border flex items-center gap-2 transition-all duration-300 ${
                    password === confirmPassword 
                      ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-600 dark:text-emerald-400' 
                      : 'bg-rose-500/10 border-rose-500/30 text-rose-600 dark:text-rose-450 animate-pulse'
                  }`}>
                    {password === confirmPassword ? (
